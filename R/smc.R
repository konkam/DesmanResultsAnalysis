...
smc_kernel_i<-function(i,g,n_vsa,tau_ivga,pi_igs,epsilon_iba,alpha_g,delta,g_neq_g =g_neq_g_f(g)){
  ...
  pi_gs=sampler_pi_i(mu_ivsag = mu_ivsab,alpha_g = alpha_g)
  tau_vga=sampler_tau_i(n_vsa = n_vsa,tau_ivga = tau_ivga,pi_igs = pi_igs,epsilon_iba = epsilon_iba,g_neq_g=g_neq_g )
...
data_tempering_stratified<-function(n_vsa,seed=1,n_vsa_df=reorder_counts(n_vsa,seed=seed),lambda){
  df=n_vsa_df|>
  ...
  #'n_vsa_lambda=sub_sample_counts(n_vsa_df,lambda=.5)
  #'tau_ivga=abind::abind(sim1$tau_vga,sim2$tau_vga,along=4)
  #'pi_igs=abind::abind(sim1$pi_gs,sim2$pi_gs,along=3)
  ...
  smc_new_setup<-function(ess_min,
    ...
    if(smc_ess(n_vsa_lambda,tau_ivga,pi_igs,epsilon_iba)>= ess_min|n_unaccounted_for==0){
  ...
  update_lambda<-function(ess_min,
    ...
    if(min_samplesize==max_samplesize){
      ...
      n_vsa_lambda=data_tempering_stratified(n_vsa=n_vsa,n_vsa_df=n_vsa_df,lambda=lambda)
      sol=list(n_vsa_lambda=n_vsa_lambda,
               lambda=lambda)
    }else{
