...
mcmc_very_relax_model_string_f <- function(block_rho=TRUE,gs="jags") {
paste0(
      "
model {
...
mcmc_relax_model_string_f <- function(block_rho=TRUE,gs="jags") {
    paste0(
      "
model {
  # Likelihood
...
mcmc_very_relax_run <- function(n_vsa,
  ...
  model_string <- mcmc_very_relax_model_string_f(gs=gs,block_rho = block_rho)
  data_list <- list(
    V = V,G = G,S = S,      
    n_vsa = n_vsa,
