...
\newcommand{\afaire}[1]{\todo[linecolor=red,backgroundcolor=orange!25,bordercolor=red]{#1}}
\usepackage{stmaryrd}
...
\For {$v\in\llbracket 1,V \rrbracket$, $g\in\llbracket 1,G \rrbracket$, $s\in\llbracket 1,S \rrbracket$, $a,b,\in\llbracket 1,4\rrbracket$},
\State $\chi_{v,s,a,b,g}\gets\tau_{v,g,b}\times\pi_{g,s}\times \epsilon_{b,a}${\hfill \code{tau\_gamma\_eta}}
% \State $M_{v,s,a,b,\indexsum{g}}\gets\sum_{g\in\llbracket 1,G\rrbracket}M_{v,s,a,b,g}${\hfill \code{tau\_gamma\_eta\_gsum}}
\EndFor
\For {$v=1..V$}{} 
    \For {$s=1..S$}{} 
        % \State $M_{v,s,a,\indexsum{b},\indexsum{g}}=\sum_{b\in\llbracket 1,4\rrbracket} M_{v,s,a,b,\indexsum{g}}${\hfill\code{trans\_matrix}}
        \For {$a=1..4$}{} 
...
\EndFor    
\EndStep
Return $\left(\tau^{(i)},\pi^{(i)},\epsilon^{(i)},\mu^{(i)},E^{(i)},e^{(i)}\right)_{i\in\llbracket 1,I\rrbracket}$
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
%\item[(h)]     $\epsilon_{v,b,c}=\rho_{v,g,a}$ if $\tau_{v,g,b}=1$, and $a=c$, 
...
\tiny
\begin{NiceTabular}{X[1]X[5]X[5]X[5]X[14]X[5]X[14]}[hvlines]
&Variable&\Block{1-5}{Models}\\
   && \cellcolor{blue!10}Model 1 Desman &\cellcolor{green!10} Model 2 Block Sampling &\cellcolor{orange!10} Model 5 Tempering& \cellcolor{yellow!20}Model 3 Fixed variants & \cellcolor{red!10}Model 4 Relaxation  \\
$\star$&$G$&\Block[fill=blue!10]{1-5}{$\mathrm{Dirac}(G^\star)$}\\
\faEye&$V,~S,~n_{\indexvec{v},\indexvec{s},\indexsum{a}} $&\Block[fill=blue!10]{1-5}{observed, exogenous}\\
$\star$&$\alpha_\tau$&\Block{1-2}-&&\cellcolor{orange!10}{fixed}&\Block{1-2}-&\\
$\star$/&$\tau\mid\alpha_\tau$&\Block[fill=blue!10]{1-2}{$\tau_{v,g,\indexvec{b}}\sim \mathrm{Uniform}(\{A,C,G,T\})$} && \cellcolor{orange!10}{$\forall v,g,~\tau_{v,g,\indexvec{b}}\overset{\text{iid}}\sim$ $\mathrm{Dirichlet}(\alpha_\tau\mathds{1}_4)$}& \cellcolor{yellow!20}{$\mathrm{Dirac}_{\tau^\star}$} & -\\
$\star$&$\alpha_\epsilon$&\cellcolor{blue!10}{$\mathrm{Dirac}(0.1 \mathds{1}_4)$}&\Block[fill=green!10]{1-3}{$\mathrm{Dirac}(\alpha_\epsilon^\star)$}&&&-\\\hline
&$\tilde\epsilon\mid\alpha_\epsilon$&-&\Block[fill=green!10]{1-3}{$\mathrm{Beta}(\alpha_\epsilon)$}&&&-\\\hline
&$\epsilon\mid\alpha_\epsilon,{\alpha}_\epsilon$&\cellcolor{blue!10}{$\forall b, \epsilon_{b,\indexvec{a}}\sim$ $\mathrm{Dirichlet}(\alpha_\epsilon)$}&\Block[fill=green!10]{1-3}{$\mathrm{Dirac}\left(\tilde\epsilon/3 (J_4-I_4)+(1-\tilde\epsilon) I_4\right)$}&&&-\\\hline
$\star$&${\shape}_\rho$&\Block{1-4}-&&&&$\cellcolor{red!10}{\mathrm{Dirac}({\shape}_\rho^\star)}$\\\hline
&$\alpha_\rho$&\Block{1-4}-&&&&$\cellcolor{red!10}{\mathrm{Gamma}({\shape}_\rho)}$\\\hline
&$\rho\mid \alpha_\rho$&\Block[fill=blue!10]{1-4}{$\mathrm{Dirac}\left(\left(\sum_{b\in\llbracket 1,4\rrbracket}\epsilon_{b,a}\tau_{v,g,b}\right)_{a\in\llbracket 1,4\rrbracket}\right)$}&&&&\cellcolor{red!10}{$\forall v,g,~\rho_{v,g,\indexvec{a}}\sim$ $\mathrm{Dirichlet}(\alpha_\rho\mathds{1}_4)$}\\\hline
$\star$&${\shape}_\pi$&-&\Block[fill=green!10]{1-4}{$\mathrm{Dirac}(\shape_\pi^\star)$}&&&\\\hline
&$\alpha_\pi$&\cellcolor{blue!10}{$\mathrm{Dirac}(\alpha_\pi^\star)$}&\Block[fill=green!10]{1-4}{$\mathrm{Beta}({\shape}_\pi)$}&&&\\\hline
&$\pi$&\Block[fill=blue!10]{1-5}{$\forall g,~ \pi_{\indexvec{g},s}\sim $ $\mathrm{Dirichlet}(\alpha_\pi \mathds{1}_G)$}&&&&\\\hline
&$\chi\mid\pi,\epsilon,\tau$&\Block[fill=blue!10]{1-4}{$\forall v,s,a,b,g~ \chi_{v,s,a,b,g}\sim $ $\mathrm{Dirac}(\rho_{v,g,a}\pi_{g,s}\epsilon_{b,a})$}&&&&-\\\hline
&$\chi_{\indexvec{v},\indexvec{s},\indexvec{a},\indexsum{b},\indexvec{g}}\mid\rho,\pi$&\Block{1-4}{}&&&&\cellcolor{red!10}{$\forall v,s,a,g$ $ \chi_{v,s,a,\indexsum{b},g}\sim $ $\mathrm{Dirac}(\rho_{v,g,a}\pi_{g,s})$}\\\hline
&$\countdetail\mid n,\chi$&\Block[fill=blue!10]{1-4}{$\forall v,s, a,~\countdetail_{v,s,a,\indexvec{b},\indexvec{g}}\sim \mathrm{Multinomial}\left(n_{v,s,a},\chi_{v,s,a,\indexvec{b},\indexvec{g}}\right)$ }&&&&-\\\hline
&$\countdetail_{\indexvec{v},\indexvec{s},\indexvec{a},\indexsum{b},\indexvec{g}}\mid n,\chi$&\Block{1-4}{}&&&&\cellcolor{red!10}{$\forall v,s, \countdetail_{v,s,a,\indexsum{b},\indexvec{g}}\sim $ $\mathrm{Multinomial}$ $\left(n_{v,s,a},\chi_{v,s,a,\indexsum{b},\indexvec{g}}\right)$ }\\\hline
\faEye&$n\mid \countdetail$&\Block[fill=blue!10]{1-5}{$ \mathrm{Dirac}\left(\countdetail_{v,s,a,\indexsum{b},\indexsum{g}}\right)$ }

\end{NiceTabular}
\end{table}


\subsection{Summary of custom kernels}
...
&Variable&\Block{1-5}{Models}\\
   && \cellcolor{blue!10}Model 1 Desman &\cellcolor{green!10} Model 2 Block Sampling &\cellcolor{orange!10} Model 5 Tempering& \cellcolor{yellow!20}Model 3 Fixed variants & \cellcolor{red!10}Model 4 Relaxation  \\
   &$\countdetail\mid n,\chi$&\Block[fill=blue!10]{1-4}{$\mathrm{Sampler}_m:$ $\forall v,s, a,~\countdetail_{v,s,a,\indexvec{b},\indexvec{g}}\sim \mathrm{Multinomial}\left(n_{v,s,a},\chi_{v,s,a,\indexvec{b},\indexvec{g}}\right)$ }&&&&-\\\hline

&$\countdetail_{\indexvec{v},\indexvec{s},\indexvec{a},\indexsum{b},\indexvec{g}}\mid n,\chi$&\Block{1-4}{}&&&&
  \cellcolor{red!10}{$\mathrm{Sampler}_{m_{\indexsum{b}}}:$ $\forall v,s, \countdetail_{v,s,a,\indexsum{b},\indexvec{g}}\sim \mathrm{Multinomial}\left(n_{v,s,a},\chi_{v,s,a,\indexsum{b},\indexvec{g}}\right)$ }\\\hline
&$\tau\mid\countdetail,\alpha_\tau$&
   \cellcolor{blue!20}{$\tau_{v,g,\indexvec{b}}$ $\sim \mathrm{Multinomial}$} &
   \cellcolor{green!10}{$\tau_{v,,\indexvec{g},\indexvec{b}}$ $\sim \mathrm{Multinomial}$}& 
   \cellcolor{orange!10}{$\forall v,g,~\tau_{v,g,\indexvec{b}}\sim$ $\mathrm{Dirichlet}(\alpha_\tau\mathds{1}_4+\countdetail_{v,\bar{s},\indexvec{b},\bar{a},g})$}& 
   \cellcolor{yellow!20}{$\mathrm{Dirac}_\mathcal{T}$} & -\\
$\star$&$G$&\Block[fill=blue!10]{1-5}{$\mathrm{Dirac}(G^\star)$}\\
\faEye&$V,~S,~n_{\indexvec{v},\indexvec{s},\indexsum{a}} $&\Block[fill=blue!10]{1-5}{observed, exogenous}\\
$\star$&$\alpha_\tau$&\Block{1-2}-&&\cellcolor{orange!10}{fixed}&\Block{1-2}-&\\
$\star$/&$\tau\mid\alpha_\tau$&\Block[fill=blue!10]{1-2}{$\tau_{v,g,\indexvec{b}}\sim \mathrm{Uniform}(\{A,C,G,T\})$} && \cellcolor{orange!10}{$\forall v,g,~\tau_{v,g,\indexvec{b}}\overset{\text{iid}}\sim$ $\mathrm{Dirichlet}(\alpha_\tau\mathds{1}_4)$}& \cellcolor{yellow!20}{$\mathrm{Dirac}_{\tau^\star}$} & -\\
